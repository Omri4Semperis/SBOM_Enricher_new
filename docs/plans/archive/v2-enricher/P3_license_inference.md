...
import): subprocess `claude -p <prompt> --model <model> --allowedTools WebSearch,WebFetch --output-format json [--json-schema <schema>]`, `cwd = Path.home()`, async via `asyncio.create_subprocess_exec`. Non-zero exit →
...
config:
  ...
  - After retries exhaust → stage fails closed: `inferred_license_name = UNKNOWN`, reason in Story + (later) extended CSV; only that component
...
- `src/claude_client.py` — create: async `infer_license(purl, lib_name, version, model) -> dict` (the CLI call + JSON parse + contract validation).
...
- Steps: `src/retry.py` — async `with_retries(fn, *, transient_attempts=3, parse_attempts=2, classify)` sleeping 2s then uniform[3,8] for transient, 1s
