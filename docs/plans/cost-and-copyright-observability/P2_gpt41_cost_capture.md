...
- `src/equality.py`: `EqResult(verdict, reason)` frozen. `_judge(client, system, user)` is the only GPT call; `_text_ladder` short-circuits on `identical` /
...
`add_call(cost_usd=compute_cost("gpt-4.1", prompt_tokens, completion_tokens, cached_tokens), raw=content)` **before** `_parse_json_content` — so a
...
empty meta.
- Verify: `.\.venv\Scripts\python.exe -m pytest tests/test_equality.py tests/test_copyright.py -q` → exit 0 (new asserts: judge verdict carries a
...
- Verify: `.\.venv\Scripts\python.exe -m pytest tests/test_results_csv.py tests/test_pipeline.py -q` → exit 0. Then full suite (Validation gate).
- Commit when green.
...
To abandon this phase: `git reset --hard {baseline hash from PLAN.md's phase table}`, then set this phase's Status to `blocked` in `PLAN.md` with a one-line
