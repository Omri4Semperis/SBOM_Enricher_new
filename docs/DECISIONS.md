...
## 2026-07-19 — v2 requirements grilling (enriched CSV, license files, URL quality, dedup, project layout)

Signed off by Omri. Scope: five requirements grilled to shared understanding.

ADRs written where the three criteria held: **0011** (A), **0012** (B),
**0013** (D). C (project layout) and E (prompt-only URL quality) stay here —
easy to reverse; E's durable meaning is in CONTEXT under Inferred License Code
URL.
...
### C. License file layout by `project_name`
...
- F2. Cache stays project-agnostic (keyed by `component_name`); on a hit, restore
  obeys this run's layout (C2/C3 per-project dirs / `_misc` / flat).
...
- E-risk: prompt-only has no safety net if Claude still picks boilerplate.
  Accepted. Owner: Omri — revisit if it recurs.
...
- docs/archive/DECISIONS_2026-07-16_freeze-prevention.md
