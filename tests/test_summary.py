...
def test_fixture_run_extended_csv_and_summary(tmp_path, monkeypatch):
    ...
    assert costs["license_inference"]["total_usd"] == UNKNOWN_COST
...
def test_build_summary_cache_hit_reduces_unknown(tmp_path):
    ...
    assert payload["costs"]["license_inference"]["unknown_cost_calls"] == 1
