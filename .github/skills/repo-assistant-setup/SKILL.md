...
metadata:
  version: 26-07-08-1
  provenance: Original to this library. Ported to Cursor (.github/skills, AGENTS.md, .github/hooks.json).
...
`<repo>/.github/skills/git-hygiene/` and merge its `hooks-snippet.json` into
`<repo>/.github/hooks.json` (create with `"version": 1` if missing). It stays
a TEMPLATE here (never a directory directly under `~/.github/skills/`): a
