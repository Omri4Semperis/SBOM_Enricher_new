...
- The `beforeShellExecution` hook (see `hooks-snippet.json` in this directory;
  merge into the target repo's `.github/hooks.json` on install) blocks
  agent-made `git commit`s when junk is staged and points here. It guards
  only commits made through the agent — the user's own terminal commits
  bypass it. The hook is the one place the scanner runs git itself (read-only,
  fixed args, no shell): a commit hook fires non-interactively and can't pause
  for command approval the way the interactive scan above does.
  Model-side procedure above remains the guarantee if hooks are not installed.
